...
def run():
    st.title("🏆 컨벤션 계산기")
    st.caption("수금 실적을 업로드하면 수금자별 컨벤션 환산금액과 달성 조건을 확인할 수 있습니다.")

    with st.expander("계산 기준 확인", expanded=False):
        st.markdown(
            f"""
            **사용 방법**
            1. 한화라이프랩 전산의 계약관리에서 보유계약 장기 목록을 내려받습니다.
            2. 엑셀 파일을 업로드하고 수금자별 결과를 확인합니다.
            3. 전체 결과 엑셀을 다운로드합니다.

            **달성 기준**
            - 일반: **{CONVENTION_GENERAL_TARGET:,.0f}원**
            - 더블: **{CONVENTION_DOUBLE_TARGET:,.0f}원**
            - 트리플: **{CONVENTION_TRIPLE_TARGET:,.0f}원**
            - 필수 조건: 인정 건수 **{CONVENTION_MIN_COUNT}건 이상**, 한화생명
              **{CONVENTION_HANWHA_MIN_PREMIUM:,.0f}원 이상 1건**
    ...
    st.subheader("1. 수금 실적 업로드")
    uploaded_file = st.file_uploader(
        "컨벤션 계산용 엑셀 파일",
        type=["xlsx"],
        help="엑셀 파일(.xlsx)만 업로드할 수 있습니다.",
    )
    if not uploaded_file:
        st.info("계약 목록 엑셀 파일을 업로드해 주세요.")
        return
    ...
    try:
        with st.spinner("수금 실적을 계산하고 있습니다..."):
            raw = load_df(uploaded_file)
            df_valid, excluded_df = exclude_contracts(raw)
            excluded_disp_all = build_excluded_with_reason(excluded_df)
            missing = check_required_columns(df_valid)
    ...
    missing_share_df = df[df["쉐어율미입력"]].copy()
    review_count = int(
        (df["계약일자_raw"].isna() | df["쉐어율미입력"]).sum()
    )

    st.subheader("2. 업로드 결과")
    summary_cols = st.columns(4)
    summary_cols[0].metric("전체 계약", f"{len(raw):,}건")
    summary_cols[1].metric("계산 포함", f"{len(df):,}건")
    summary_cols[2].metric("계산 제외", f"{len(excluded_df):,}건")
    summary_cols[3].metric("확인 필요", f"{review_count:,}건")

    issues = []
    if not invalid_dates.empty:
        issues.append(f"계약일자를 인식하지 못한 계약이 {len(invalid_dates):,}건 있습니다.")
    if not missing_share_df.empty:
        issues.append(
            f"쉐어율 미입력 계약 {len(missing_share_df):,}건은 건수 및 필수조건 계산에서 제외됩니다. "
            "단독 계약은 쉐어율에 100을 입력해 주세요."
        )
    if issues:
        with st.expander("⚠️ 확인이 필요한 항목", expanded=True):
            for message in issues:
                st.warning(message)

    if not excluded_df.empty:
        with st.expander(f"계산에서 제외된 계약 {len(excluded_df):,}건", expanded=False):
            st.dataframe(excluded_disp_all, use_container_width=True, hide_index=True)

    if df.empty:
        st.warning("제외 기준을 적용한 뒤 계산할 계약이 없습니다.")
        return

    collectors = sorted(df["수금자명"].astype(str).unique().tolist())
    if not collectors:
        st.warning("수금자명이 입력된 계약이 없습니다.")
        return

    st.subheader("3. 수금자별 결과")
    selected = st.selectbox("결과를 확인할 수금자를 선택하세요", collectors)
    show_df = df[df["수금자명"].astype(str) == selected].copy()
    req = check_convention_requirements(show_df)
    conv_sum = req["컨벤션환산금액"]

    if not req["필수조건"]:
        next_goal = "필수조건 확인"
    elif conv_sum < CONVENTION_GENERAL_TARGET:
        next_goal = f"일반까지 {CONVENTION_GENERAL_TARGET - conv_sum:,.0f}원"
    elif conv_sum < CONVENTION_DOUBLE_TARGET:
        next_goal = f"더블까지 {CONVENTION_DOUBLE_TARGET - conv_sum:,.0f}원"
    elif conv_sum < CONVENTION_TRIPLE_TARGET:
        next_goal = f"트리플까지 {CONVENTION_TRIPLE_TARGET - conv_sum:,.0f}원"
    else:
        next_goal = "트리플 기준 달성"

    result_cols = st.columns(4)
    result_cols[0].metric("컨벤션 환산금액", f"{conv_sum:,.0f}원")
    result_cols[1].metric("최종 달성 등급", req["달성등급"])
    result_cols[2].metric("인정 건수", f"{req['인정건수']:g}건")
    result_cols[3].metric("다음 확인 항목", next_goal)

    if req["금액수준"] != req["달성등급"]:
        st.warning(
            f"금액은 {req['금액수준']} 수준이지만 필수 조건을 충족하지 못해 "
            f"최종 달성 등급은 {req['달성등급']}입니다."
        )
    ...
    with condition_cols[0]:
        message = (
            f"인정 건수 {'충족' if req['건수5건'] else '미충족'} · "
            f"현재 {req['인정건수']:g}건 / 기준 {CONVENTION_MIN_COUNT}건"
        )
        st.success(message) if req["건수5건"] else st.warning(message)
    with condition_cols[1]:
        message = (
            f"한화생명 조건 {'충족' if req['한화생명5만'] else '미충족'} · "
            f"{CONVENTION_HANWHA_MIN_PREMIUM:,.0f}원 이상 1건"
        )
        st.success(message) if req["한화생명5만"] else st.warning(message)

    with st.expander("전체 등급 기준 보기", expanded=False):
        grade_table = pd.DataFrame(
            {
                "등급": ["일반", "더블", "트리플"],
                "목표금액": [
                    f"{CONVENTION_GENERAL_TARGET:,.0f}원",
                    f"{CONVENTION_DOUBLE_TARGET:,.0f}원",
                    f"{CONVENTION_TRIPLE_TARGET:,.0f}원",
                ],
                "금액기준": [
                    "충족" if conv_sum >= CONVENTION_GENERAL_TARGET else "미충족",
                    "충족" if conv_sum >= CONVENTION_DOUBLE_TARGET else "미충족",
                    "충족" if conv_sum >= CONVENTION_TRIPLE_TARGET else "미충족",
                ],
                "최종달성": [
                    "달성" if req["일반달성"] else "미달성",
                    "달성" if req["더블달성"] else "미달성",
                    "달성" if req["트리플달성"] else "미달성",
                ],
            }
        )
        st.dataframe(grade_table, use_container_width=True, hide_index=True)

    with st.expander(f"{selected} 계약 상세 {len(show_df):,}건", expanded=False):
        st.dataframe(to_styled(show_df), use_container_width=True)

    st.subheader("4. 전체 수금자 현황")
    group = make_group(df)
    st.dataframe(
        format_group_for_display(group),
        use_container_width=True,
        hide_index=True,
    )
    with st.expander(f"전체 계약 상세 {len(df):,}건", expanded=False):
        st.dataframe(to_styled(df), use_container_width=True)

    st.subheader("5. 결과 파일 다운로드")
    with st.spinner("결과 엑셀을 준비하고 있습니다..."):
        wb = build_workbook(df, group, excluded_disp_all)
        excel_output = BytesIO()
        wb.save(excel_output)
        excel_output.seek(0)
