...
def run():
    ...
    if st.sidebar.button("📊 비교 시작"):
        ...
        st.subheader("🌟 리모델링을 통해 기대할 수 있는 효과는 다음과 같습니다.")
        기대효과_후보 = []

        if fee_diff < 0:
            기대효과_후보.append("💸 보험료가 줄어들어 가계 부담이 완화됩니다.")
        elif fee_diff > 0 and (강화 + 신규) > 0:
            기대효과_후보.append("📈 보험료는 증가했지만, 보장이 강화되어 실질적인 대비가 가능합니다.")

        if total_diff < 0:
            기대효과_후보.append("💰 총 납입 보험료가 증가했지만, 장기 리스크를 해소할 수 있는 설계입니다.")
        elif total_diff > 0:
            기대효과_후보.append("🎯 총 보험료가 줄어들어 재무 여유가 생깁니다.")

        if year_diff < 0:
            기대효과_후보.append("🕰️ 납입기간은 길어졌지만 월 부담은 낮아졌습니다.")
        elif year_diff > 0:
            기대효과_후보.append("🚀 납입기간 단축으로 자산 완결 시점이 앞당겨졌습니다.")

        if 신규 > 0:
            기대효과_후보.append(f"🌱 신규 보장 {신규}개 추가로 다양한 리스크를 대비합니다.")
        if 강화 > 0:
            기대효과_후보.append(f"🛡️ 보장 강화 {강화}개로 위기 대응 능력이 향상되었습니다.")
        if 삭제 > 0:
            기대효과_후보.append("🧹 중복/불필요한 특약 제거로 구조가 단순해졌습니다.")

        기대효과_후보.append("🔍 본인의 상황에 맞춘 리모델링으로 설계 만족도가 높습니다.")

        st.markdown("#### ✨ 리모델링을 통해 기대할 수 있는 효과는 다음과 같습니다:")
        for 문장 in random.sample(기대효과_후보, min(6, len(기대효과_후보))):
            st.markdown(f"- {문장}")
