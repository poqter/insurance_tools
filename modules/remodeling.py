...
def sentence_candidates(analysis: AnalysisResult, changes: pd.DataFrame, goal: str, priorities: list[str], keep_existing: bool, old_monthly: int, new_monthly: int) -> dict[str, list[str]]:
    ...
    if analysis.price_direction == "감소" and analysis.coverage_direction in {"강화", "강화와 조정 혼합"}:
        ...
        headline = ["보험료와 보장 변화를 함께 고려해 현재 상황에 맞게 재구성한 제안입니다."]
...
def _change_block(title: str, old: str, new: str, result: str) -> str:
    color, bg = "#2E7D5B", "#E8F5EE"
    if any(word in result for word in ["감소", "축소", "삭제", "정리", "단축"]):
        color, bg = "#B96B26", "#FFF2E6"
    elif any(word in result for word in ["유지", "동일"]):
        color, bg = "#17365D", "#EAF2F8"
    return f"""
    <div style="border:1px solid #D7DDE5;border-radius:12px;padding:14px 14px;background:#FFFFFF;min-height:154px;box-shadow:0 2px 8px rgba(16,42,67,.05);">
      <div style="font-size:15px;color:#17365D;font-weight:800;margin-bottom:12px;">{title}</div>
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
        <div style="width:42%;font-size:13px;color:#667085;text-align:center;"><div style="margin-bottom:4px;">기존</div><strong style="font-size:16px;color:#344054;">{old}</strong></div>
        <div style="font-size:22px;color:#98A2B3;">→</div>
        <div style="width:42%;font-size:13px;color:#667085;text-align:center;"><div style="margin-bottom:4px;">변경 후</div><strong style="font-size:16px;color:#2F75B5;">{new}</strong></div>
      </div>
      <div style="margin-top:14px;padding:7px 9px;border-radius:8px;background:{bg};color:{color};font-size:14px;font-weight:800;text-align:center;">{result}</div>
    </div>
    """


def render_preview(title: str, proposal_label: str, headline: str, analysis: AnalysisResult, old_monthly: int, new_monthly: int, old_total: int | None, new_total: int | None, changes: pd.DataFrame, contracts: pd.DataFrame, recommendation: str, caution: str) -> None:
    left, right = st.columns([3, 1])
    with left:
        st.markdown(f"## {title}")
    with right:
        st.markdown(f"<div style='margin-top:12px;padding:9px 12px;border-radius:9px;background:#168A8A;color:white;font-weight:800;text-align:center;'>{proposal_label}</div>", unsafe_allow_html=True)
    ...
    if not core.empty:
        ...
        rows = list(core.head(4).iterrows())
        for start in range(0, len(rows), 2):
            cols = st.columns(2)
            for col, (_, row) in zip(cols, rows[start:start+2]):
                with col:
                    st.markdown(_change_block(
                        normalize_text(row.get("변경할 보장")),
                        normalize_text(row.get("기존에는")) or "-",
                        normalize_text(row.get("변경 후에는")) or "-",
                        normalize_text(row.get("어떻게 달라지나요? [목록 선택]")) or "변경",
                    ), unsafe_allow_html=True)

    st.markdown("### 계약 정리 요약")
    if contracts.empty:
        st.caption("계약 처리 방향을 입력하면 유지·조정·신규 관련 요약이 표시됩니다.")
    else:
        directions = contracts["처리 방향 [목록 선택]"].fillna("").astype(str)
        keep_count = int(directions.str.contains("유지").sum())
        adjust_count = int(directions.str.contains("감액|조정|해지|결정").sum())
        check_count = int(directions.str.contains("확인").sum())
        s1, s2, s3 = st.columns(3)
        s1.metric("유지", f"{keep_count}건")
        s2.metric("조정·검토", f"{adjust_count}건")
        s3.metric("추가 확인", f"{check_count}건")
        summary = contracts[["기존 계약/보장", "처리 방향 [목록 선택]"]].head(4).rename(columns={"기존 계약/보장":"계약/보장", "처리 방향 [목록 선택]":"처리 방향"})
        st.dataframe(summary, use_container_width=True, hide_index=True)

    if headline:
        st.markdown(f"<div style='margin-top:14px;padding:13px 16px;border-left:5px solid #17365D;background:#F7F9FC;font-size:16px;font-weight:800;color:#17365D;'>{headline}</div>", unsafe_allow_html=True)
...
def _contract_counts(contracts: pd.DataFrame) -> tuple[int, int, int]:
    if contracts.empty:
        return 0, 0, 0
    directions = contracts["처리 방향 [목록 선택]"].fillna("").astype(str)
    keep = int(directions.str.contains("유지").sum())
    adjust = int(directions.str.contains("감액|조정|해지|결정").sum())
    check = int(directions.str.contains("확인").sum())
    return keep, adjust, check
...
def create_excel(*, customer_name: str, title: str, proposal_label: str, consultation_date: date, consultant: str,
    ...
    ws = wb.active
    ws.title = "01_한눈에보기"
    for col, width in {"A": 16, "B": 16, "C": 16, "D": 16, "E": 3, "F": 16, "G": 16, "H": 16, "I": 16, "J": 3}.items():
        ws.column_dimensions[col].width = width

    _merge_write(ws, "A1:G2", title, font=Font(name="맑은 고딕", size=20, bold=True, color=NAVY_DARK), alignment=Alignment(horizontal="left", vertical="center"))
    _merge_write(ws, "H1:J2", proposal_label, font=Font(name="맑은 고딕", size=11.5, bold=True, color=WHITE), fill=PatternFill("solid", fgColor=TEAL), alignment=Alignment(horizontal="center", vertical="center"), border=Border(left=THIN_GRAY, right=THIN_GRAY, top=THIN_GRAY, bottom=THIN_GRAY))
    ...
    for (label, main, sub, direction), rng in zip(cards, ranges):
        ...
        _merge_write(ws, rng, f"{label}\n{main}\n{sub}", font=Font(name="맑은 고딕", size=15, bold=True, color=font_color), fill=PatternFill("solid", fgColor=fill), alignment=Alignment(horizontal="center", vertical="center", wrap_text=True), border=Border(left=THIN_GRAY, right=THIN_GRAY, top=THIN_GRAY, bottom=THIN_GRAY))
    ...
    start_rows = [8, 8, 13, 13]
    col_ranges = [("A", "E"), ("F", "J"), ("A", "E"), ("F", "J")]
    for idx, (_, item) in enumerate(core.iterrows()):
        r = start_rows[idx]
        c1, c2 = col_ranges[idx]
        change_text = normalize_text(item.get("어떻게 달라지나요? [목록 선택]")) or "변경"
        color, fill = _change_color(change_text)
        _merge_write(ws, f"{c1}{r}:{c2}{r}", normalize_text(item.get("변경할 보장")), font=Font(name="맑은 고딕", size=12, bold=True, color=NAVY_DARK), fill=PatternFill("solid", fgColor=GRAY), alignment=Alignment(horizontal="left", vertical="center"), border=Border(left=THIN_GRAY, right=THIN_GRAY, top=THIN_GRAY, bottom=THIN_GRAY))
        _merge_write(ws, f"{c1}{r+1}:{c2}{r+2}", f"기존  {normalize_text(item.get('기존에는')) or '-'}\n→  변경 후  {normalize_text(item.get('변경 후에는')) or '-'}", font=Font(name="맑은 고딕", size=11.5, bold=True, color=BLACK), fill=PatternFill("solid", fgColor=WHITE), alignment=Alignment(horizontal="center", vertical="center", wrap_text=True), border=Border(left=THIN_GRAY, right=THIN_GRAY, top=THIN_GRAY, bottom=THIN_GRAY))
        _merge_write(ws, f"{c1}{r+3}:{c2}{r+3}", change_text, font=Font(name="맑은 고딕", size=11.5, bold=True, color=color), fill=PatternFill("solid", fgColor=fill), alignment=Alignment(horizontal="center", vertical="center"), border=Border(left=THIN_GRAY, right=THIN_GRAY, top=THIN_GRAY, bottom=THIN_GRAY))
    ...
    if len(core) < 4:
        for idx in range(len(core), 4):
            r = start_rows[idx]
            c1, c2 = col_ranges[idx]
            _merge_write(ws, f"{c1}{r}:{c2}{r+3}", "", fill=PatternFill("solid", fgColor=WHITE), border=Border(left=THIN_GRAY, right=THIN_GRAY, top=THIN_GRAY, bottom=THIN_GRAY))

    keep_count, adjust_count, check_count = _contract_counts(contracts)
    _merge_write(ws, "A18:J18", "계약 정리 요약", font=Font(name="맑은 고딕", size=12, bold=True, color=WHITE), fill=PatternFill("solid", fgColor=NAVY), alignment=Alignment(horizontal="left", vertical="center"))
    _merge_write(ws, "A19:C20", f"유지\n{keep_count}건", font=Font(name="맑은 고딕", size=13, bold=True, color=BLUE), fill=PatternFill("solid", fgColor=LIGHT_BLUE), alignment=Alignment(horizontal="center", vertical="center", wrap_text=True), border=Border(left=THIN_GRAY,right=THIN_GRAY,top=THIN_GRAY,bottom=THIN_GRAY))
    _merge_write(ws, "D19:F20", f"조정·검토\n{adjust_count}건", font=Font(name="맑은 고딕", size=13, bold=True, color=ORANGE), fill=PatternFill("solid", fgColor=LIGHT_ORANGE), alignment=Alignment(horizontal="center", vertical="center", wrap_text=True), border=Border(left=THIN_GRAY,right=THIN_GRAY,top=THIN_GRAY,bottom=THIN_GRAY))
    _merge_write(ws, "G19:J20", f"추가 확인\n{check_count}건", font=Font(name="맑은 고딕", size=13, bold=True, color=NAVY), fill=PatternFill("solid", fgColor=LIGHT_GOLD), alignment=Alignment(horizontal="center", vertical="center", wrap_text=True), border=Border(left=THIN_GRAY,right=THIN_GRAY,top=THIN_GRAY,bottom=THIN_GRAY))
    ws.row_dimensions[19].height = 27
    ws.row_dimensions[20].height = 27

    r = 21
    for _, item in contracts.head(4).iterrows():
        _merge_write(ws, f"A{r}:F{r}", normalize_text(item.get("기존 계약/보장")), font=Font(name="맑은 고딕", size=11, bold=True, color=BLACK), fill=PatternFill("solid", fgColor=WHITE), alignment=Alignment(horizontal="left", vertical="center", wrap_text=True), border=Border(bottom=THIN_GRAY))
        _merge_write(ws, f"G{r}:J{r}", normalize_text(item.get("처리 방향 [목록 선택]")), font=Font(name="맑은 고딕", size=11, bold=True, color=NAVY), fill=PatternFill("solid", fgColor=LIGHT_BLUE), alignment=Alignment(horizontal="center", vertical="center", wrap_text=True), border=Border(bottom=THIN_GRAY))
    ...
    if headline:
        _merge_write(ws, "A25:J26", headline, font=Font(name="맑은 고딕", size=11.5, bold=True, color=NAVY_DARK), fill=PatternFill("solid", fgColor="F7F9FC"), alignment=Alignment(horizontal="left", vertical="center", wrap_text=True), border=Border(left=MEDIUM_NAVY))
    _merge_write(ws, "A27:F27", f"상담일: {consultation_date:%Y-%m-%d}", font=Font(name="맑은 고딕", size=9.5, color=GRAY_DARK), alignment=Alignment(horizontal="left"))
    _merge_write(ws, "G27:J27", f"담당: {consultant or '-'}", font=Font(name="맑은 고딕", size=9.5, color=GRAY_DARK), alignment=Alignment(horizontal="right"))
    ws.print_area = "A1:J27"
    _page_setup(ws, 1)

    if output_type == "계약정리 포함(2페이지)" and not contracts.empty:
        ws2 = wb.create_sheet("02_계약정리")
        widths = [27, 22, 36, 36]
        for i, width in enumerate(widths, 1):
            ws2.column_dimensions[get_column_letter(i)].width = width
        _merge_write(ws2, "A1:D2", f"{customer_name}님 기존 계약 정리 및 확인사항", font=Font(name="맑은 고딕", size=20, bold=True, color=NAVY_DARK), alignment=Alignment(horizontal="left", vertical="center"))
        headers = ["기존 계약/보장", "처리 방향", "판단 근거", "진행 조건"]
        for c, header in enumerate(headers, 1):
            cell = ws2.cell(4, c, header)
        ...
        for _, item in contracts.iterrows():
            ...
            for c, value in enumerate(values, 1):
                ...
                cell.font = Font(name="맑은 고딕", size=11, bold=c in {1,2}, color=NAVY if c == 2 else BLACK)
                cell.fill = PatternFill("solid", fgColor=LIGHT_BLUE if c == 2 else WHITE)
                cell.alignment = Alignment(horizontal="left", vertical="center", wrap_text=True)
                cell.border = Border(left=THIN_GRAY, right=THIN_GRAY, top=THIN_GRAY, bottom=THIN_GRAY)
            ws2.row_dimensions[r2].height = 42
            r2 += 1
        _merge_write(ws2, f"A{r2+1}:D{r2+2}", "신규 계약의 승인 결과를 확인한 뒤 기존 계약의 유지·조정 여부를 최종 결정합니다.", font=Font(name="맑은 고딕", size=11, bold=True, color=NAVY_DARK), fill=PatternFill("solid", fgColor=LIGHT_GOLD), alignment=Alignment(horizontal="left", vertical="center", wrap_text=True), border=Border(left=MEDIUM_NAVY))
        ws2.print_area = f"A1:D{r2+2}"
        _page_setup(ws2, 1)
...
def run():
    ...
    with tabs[0]:
        ...
        with c1:
            ...
            proposal_label = st.text_input("제안 표시", value="균형 보장형 추천안", placeholder="예: 보험료 절감형 추천안", key="rp_label")
        with c2:
            ...
            output_type = st.radio("엑셀 출력", ["한눈에보기(1페이지)", "계약정리 포함(2페이지)"], horizontal=True, key="rp_output")
    ...
    with tabs[2]:
        ...
        with st.expander("기존 계약 처리 방향 입력 · 1페이지 요약 및 선택형 2페이지에 사용"):
    ...
    output_type = st.session_state.get("rp_output", "한눈에보기(1페이지)")
    ...
    with tabs[3]:
        ...
        render_preview(title, proposal_label, headline, analysis, old_monthly, new_monthly, old_total, new_total, changes, contracts, recommendation, caution)
        ...
        if customer_name and old_monthly and new_monthly and not changes.empty:
            ...
            filename = _safe_filename(f"{customer_name}님_보험리모델링_시각제안서_{consultation_date:%Y%m%d}.xlsx")
