...
def run():
    ...
    if st.sidebar.button("📊 비교 시작"):
        ...
        # --- 보험료 변화 기반 ---
        if before_fee > 0 or after_fee > 0:
            ...
            elif after_fee > before_fee:
                기대효과_후보.append("📈 월 보험료가 증가했지만, 장기적인 보장을 위한 전략적 설계일 수 있습니다.")
        ...
        if before_years > 0 or after_years > 0:
            if after_years > before_years:
                기대효과_후보.append("🕰️ 납입기간은 길어졌지만 월 부담이 낮아져 유연한 설계가 가능합니다.")
            elif after_years < before_years:
        ...
        if 기대효과_후보:
            ...
            기대효과_후보.append("📊 리모델링 변화 내역이 명확히 정리되어 의사결정에 큰 도움이 됩니다.")
